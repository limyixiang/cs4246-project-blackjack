...
class BlackjackAgent:
    def __init__(
        ...
        *,
        ...
        # Q_play[player_sum(32), dealer(11), usable(2), tc_bucket, action(5)]
        # actions: 0=stand,1=hit,2=surrender,3=double,4=split
        self.Q_play = np.zeros((32, 11, 2, self.n_tc, len(Action)), dtype=np.float32)
    ...
    def save(self, path: Path | str):
        ...
        # Ensure JSON-serializable (convert numpy scalars to built-in types)
    ...
    @staticmethod
    def _unpack(obs):
        # obs = (psum, dealer, usable, tc_idx, phase, current_hand_idx, total_hands)
    ...
    def _must_stick(self, obs):
        ...
        return len(self.base.player.active_hand) == 2 and ps == 21
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the Hi-Lo split SARSA agent and export plots.",
    )
    parser.add_argument(
        "--preplay-episodes",
        type=int,
...
for episode in tqdm(range(n_preplay_episodes), mininterval=5.0, desc="Train Q_play", disable=tqdm_disable):
    ...
    while not done:
        ...
        # If a split was previously chosen, accumulate all subsequent rewards
        if split_pending:
            split_return += reward

        if a == Action.SPLIT:
            # Defer the update for SPLIT until the end of the episode
            split_pending = True
            split_state = s
            # choose next action for continuing SARSA trajectory
            if done:
                # edge case: if environment ends immediately after split, update now
                agent.update_play_sarsa(split_state, int(Action.SPLIT), split_return, True)
                break
            a_next = agent.select_play(s_next)
        else:
            if done:
                agent.update_play_sarsa(s, int(a), reward, True)
                # if an earlier split is pending, close it with the final accumulated return
                if split_pending:
                    agent.update_play_sarsa(split_state, int(Action.SPLIT), split_return, True)
                break
            a_next = agent.select_play(s_next)
            agent.update_play_sarsa(s, int(a), reward, False, s_next, int(a_next))
...
def plot_q_play(
    ...
    show: 'delta' (Q_hit - Q_stand), 'stand', 'hit', 'surrender', 'double', 'split', or 'policy' (argmax).
    ...
    titles = {
        ...
        'double': 'Q_double',
        'split': 'Q_split',
        'policy': 'Greedy policy (0=stand,1=hit,2=surrender,3=double,4=split)'
    }
    ...
    for u, ax in enumerate(axs):
        if show == 'policy':
            ...
            M = _reorder(Mraw)
            im = ax.imshow(M, aspect='auto', cmap='tab10', vmin=0, vmax=4)
            cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04, ticks=[0, 1, 2, 3, 4])
            cbar.ax.set_yticklabels(['Stand', 'Hit', 'Surrender', 'Double', 'Split'])
        else:
            if show == 'delta':
                Mfull = Q[:, :, u, 1] - Q[:, :, u, 0]
                ...
                Mfull = Q[:, :, u, 4]
                cbar_label = 'Q value (Split)'; cmap = 'viridis'
            else:
                raise ValueError("show must be one of 'delta','stand','hit','surrender','double','split','policy'")
...
try:
    ...
    show_dd = widgets.Dropdown(options=['policy', 'delta', 'stand', 'hit', 'surrender', 'double', 'split'], value='policy', description='View')
    out = widgets.Output()

    def _update(*args):
        with out:
            out.clear_output(wait=True)
            # Save each interactive render with incremental filename (optional)
