...
class BlackjackAgent:
    def __init__(
        ...
        *,
        ...
        # Q_play[player_sum(32), dealer(11), usable(2), tc_bucket, action(3)]
        # actions: 0=stick, 1=hit, 2=surrender
        self.Q_play = np.zeros((32, 11, 2, self.n_tc, 3), dtype=np.float32)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the Hi-Lo surrender SARSA agent and export plots.",
    )
    parser.add_argument(
        "--preplay-episodes",
        type=int,
...
def plot_q_play(
    ...
    for u, ax in enumerate(axs):
        if show == 'policy':
            ...
            cbar.ax.set_yticklabels(['Stand', 'Hit', 'Surrender'])
        else:
            if show == 'delta':
                Mfull = Q[:, :, u, 1] - Q[:, :, u, 0]
