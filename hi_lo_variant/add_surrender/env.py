...
class BlackjackEnv(gym.Env):
    ...
    def __init__(self, natural = False, sab = False, num_decks: int = 1, tc_min: int = -10, tc_max: int = 10):
        ...
        # At least 3 to allow (stick, hit, surrender) in playing phase; also must cover all bet indices.
        self.action_space = spaces.Discrete(max(3, self.n_bets))
    ...
    def get_valid_actions_idxs(self):
        ...
        if len(self.player) == 2:
            return (0, 1, 2)  # stick, hit, surrender (allowed only as first decision)
        return (0, 1)  # stick, hit
    ...
    def step(self, action):
        ...
        if action == 1:  # Hit: draw card
            ...
            if is_bust(self.player):
                ...
                terminated = False
                reward = 0.0
        elif action == 0:  # Stick: resolve dealer hand
            terminated = True
            self.update_hi_lo_count(self.dealer[1]) # update hi-lo count for dealer's 2nd (unseen) card
